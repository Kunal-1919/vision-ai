...
class OfficeLocation:
    ...
    longitude: float
    radius_meters: float
    max_accuracy_meters: float = 500.0
    enabled: bool = True
    ...
    def from_file(cls, path: Path = OFFICE_LOCATION_FILE) -> "OfficeLocation":
        ...
        if not path.exists():
            default = cls(
                ...
                radius_meters=float(env_radius) if env_radius else 1500.0,
                max_accuracy_meters=float(env_max_acc) if env_max_acc else 500.0,
        ...
        return cls(
            ...
            radius_meters=float(env_radius) if env_radius else float(payload.get("radius_meters", 1500)),
            max_accuracy_meters=float(env_max_acc) if env_max_acc else float(payload.get("max_accuracy_meters", 500)),
